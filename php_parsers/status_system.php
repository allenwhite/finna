...
if (isset($_POST['action']) && $_POST['action'] == "status_post"){
	...
	if($_POST['type'] != "a" && $_POST['type'] != "b" && $_POST['type'] != "c"){
		mysqli_close($db_conx);
	    echo "type_unknown";
	    exit();
	}
	...
	if($_POST['type'] == "a"){
		...
		if(isset($_POST['friends'])){
			$frinds = $_POST['friends'].$log_username;
			$friends = array();
			$friends = explode(".", $frinds);
		}
	}
	...
	for($i = 0; $i < count($friends); $i++){
		...
		if(strlen($friend) > 0){
			...
			//put the friends into the directMessages
			if($_POST['type'] == 'c'){
				mysqli_query($db_conx, "INSERT INTO directMessages(osid, username) VALUES('$id', '$friend')");			
			}

			if($friend != $log_username){
				...
				//android push call
				$sql = "SELECT gcm_regid FROM users WHERE username='$friend' AND activated='1' LIMIT 1";
				...
				if($dbregid != "" && $dbregid != null){
					...
					sendNotification($dbregid,"Finna","$log_username is finna do something with you!","$log_username is finna do something with you!");
				}


				//ios push call
				$sql = "SELECT apnsID FROM users WHERE username='$friend' AND activated='1' LIMIT 1";
				$regid_query = mysqli_query($db_conx, $sql);
				$regid_row = mysqli_fetch_row($regid_query);
				$dbregid = $regid_row[0];
				if($dbregid != "" && $dbregid != null){
					...
					apnsPush($dbregid, "$log_username is finna do something with you!");
				}
...
//<a href="user.php?u='.$account_name.'#status_'.$id.'">///////////////////////////////////////////////////////////////////////////
?><?php 
...
if (isset($_POST['action']) && $_POST['action'] == "status_reply"){
	...
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		...
		//android push
		$sql = "SELECT gcm_regid FROM users WHERE username='$participant' AND activated='1' LIMIT 1";
		$regid_query = mysqli_query($db_conx, $sql);
		$regid_row = mysqli_fetch_row($regid_query);
		$dbregid = $regid_row[0];
		if($dbregid != "" && $dbregid != null){
			//if its set, send a notification!
			include_once("androidpush.php");
			sendNotification($dbregid,"Finna","$log_username replied to a finna!","$log_username replied to a finna!");
		}


		//ios push
		$sql = "SELECT apnsID FROM users WHERE username='$participant' AND activated='1' LIMIT 1";
		$regid_query = mysqli_query($db_conx, $sql);
		$regid_row = mysqli_fetch_row($regid_query);
		$dbregid = $regid_row[0];
		if($dbregid != "" && $dbregid != null){
			//if its set, send a notification!
			include_once("iospush.php");
			apnsPush($dbregid,"$log_username replied to a finna!");
		}
	}
	mysqli_close($db_conx);
	echo "reply_ok|$id";
...
if (isset($_POST['action']) && $_POST['action'] == "like_status"){
	...
	$statusid = preg_replace('#[^0-9]#', '', $_POST['statusid']);
	$account_name = preg_replace('#[^a-z0-9]#i', '', $_POST['user']);
	$query = mysqli_query($db_conx, "SELECT account_name FROM user_likes WHERE osid='$statusid'");
	$alreadyLiked = false;
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		$userwholiked = $row["account_name"];
		if($account_name == $userwholiked){
			$alreadyLiked = true;
		}
	}
	if(!$alreadyLiked){
		mysqli_query($db_conx, "INSERT INTO user_likes(osid, account_name, date_liked) VALUES('$statusid','$account_name',now())");
		//notifications
		$sql = "SELECT author FROM status WHERE id='$statusid' LIMIT 1";
		$query = mysqli_query($db_conx, $sql);
		$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
		$participant = $row["author"];
		
		$sql = "SELECT data FROM status WHERE id='$statusid' LIMIT 1";
		$query = mysqli_query($db_conx, $sql);
		$row = mysqli_fetch_array($query, MYSQLI_ASSOC);
		$data = $row["data"];
		$data = nl2br($data);
		...
		if($dbregid != "" && $dbregid != null){
			//if its set, send a notification!
			include_once("androidpush.php");
			sendNotification($dbregid,"Finna","$log_username said they were finna!", "$log_username said they were finna!");
		}
		...
		if($dbregid != "" && $dbregid != null){
			//if its set, send a notification!
			include_once("iospush.php");
			apnsPush($dbregid, "$log_username said they were finna!");
		}
		...
		echo "You've already said you are finna";
		exit();
	}
